...
impl<U, E> Algorithm<GradientStatus, U, E> for Adam {
    ...
    fn step(
        ...
        self.m = self.m.scale(self.beta_1) + self.g.scale(1.0 - self.beta_1);
        self.v = self.v.scale(self.beta_2) + self.g.map(|gi| gi.powi(2)).scale(1.0 - self.beta_2);
        let alpha_t = self.alpha * (1.0 - self.beta_2.powi(i_step as i32 + 1)).sqrt()
            / (1.0 - self.beta_1.powi(i_step as i32 + 1));
        self.x -= self
            .m
            .scale(alpha_t)
            .component_div(&self.v.map(|vi| vi.sqrt() + self.epsilon));
        self.f = func.evaluate_bounded(self.x.as_slice(), bounds, user_data)?;
        status.inc_n_f_evals();
    ...
    fn check_for_termination(
        ...
        self.ema_loss = self
            .beta_c
            .mul_add(prev_ema_loss, (1.0 - self.beta_c) * self.f);
        if (self.ema_loss - prev_ema_loss).abs() < self.eps_loss {
            self.ema_counter += 1;
        } else {
            self.ema_counter = 0;
        }
        if self.ema_counter >= self.patience {
...
mod tests {
    ...
    use crate::{
        algorithms::gradient::GradientStatus,
        core::{CtrlCAbortSignal, Engine, MinimizationSummary},
    ...
    fn test_problem_constructor() {
        ...
        let solver = Adam::default();
        let problem: Engine<GradientStatus, (), Infallible, MinimizationSummary> =
            Engine::new(solver);
    }
